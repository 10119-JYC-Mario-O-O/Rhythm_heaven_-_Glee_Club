...
function setKeyboardListener() {
    ...
    document.addEventListener("keyup", function (event) {
		...
		if(event.keyCode == 74){
			is_A_press = false;
			if(K3_Start[order] - 30 <= frame && frame <= K3_Start[order] + 30){
				RK3_Start.push(frame);
			}
		}
...
function update() {
	if(74 in keysDown){
		if(!is_A_press){
			if(K3_Stop[order] - 30 <= frame && frame <= K3_Stop[order] + 30){
				RK3_Stop.push(frame);
			}
		}
...
function render() {
	...
	if(TXT){
		ctx.fillStyle = "black";
		ctx.font = "64px VT323";
		ctx.fillText(`${order}`, 10, 50);
		ctx.fillText(`${score / order}`, 10, 100);
		ctx.fillText(`${K3_Sound_State}`, 10, 150);
		ctx.fillText(`${is_order_changed}`, 10, 200);
		//ctx.fillText(`${}`, 10, 250);
	}
}
